  ...
  actions: {
    nextPage: function() {
      if(!((parseInt(this.get('currentPage')) + 1) > parseInt(this.get('model.pageChecker')))) {
        this.transitionToRoute('account.user.deposits', parseInt(this.get('currentPage')) + 1);
      }
    },
    ...
    firstPage: function() {
      this.transitionToRoute('account.user.deposits', 1);
    },

    lastPage: function() {
      this.transitionToRoute('account.user.deposits', this.get('model.pageChecker'));
    }
  }
